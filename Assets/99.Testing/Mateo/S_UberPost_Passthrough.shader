    ...
    HLSLINCLUDE
        ...
        half4 FragUberPost(Varyings input) : SV_Target
        {
            ...
            #if _CHROMATIC_ABERRATION
            {
                ...
                half2 r = SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp, SCREEN_COORD_REMOVE_SCALEBIAS(uvDistorted)                ).xw;
                half2 g = SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp, SCREEN_COORD_REMOVE_SCALEBIAS(DistortUV(delta + uv)      )).yw;
                half2 b = SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp, SCREEN_COORD_REMOVE_SCALEBIAS(DistortUV(delta * 2.0 + uv))).zw;

                color = half3(r.x, g.x, b.x);
                alpha = max(r.y, max(g.y, b.y));
            }
            #else
            {
                half4 blitSample = SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp, SCREEN_COORD_REMOVE_SCALEBIAS(uvDistorted));
                color = blitSample.xyz;
                alpha = blitSample.a;
            }
            #endif
