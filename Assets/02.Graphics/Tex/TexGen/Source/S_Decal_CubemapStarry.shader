...
Shader  "HSS08/Decal/Test/Starry"
{
	Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			)
			{
				...
				float mulTime142 = _TimeParameters.x * 0.1;
				float3 ase_worldViewDir = ( _WorldSpaceCameraPos.xyz - positionWS );
				ase_worldViewDir = normalize(ase_worldViewDir);
				float simplePerlin3D141 = snoise( ( ( mulTime142 * float3(0,1,0) ) + ase_worldViewDir )*10.0 );
				simplePerlin3D141 = simplePerlin3D141*0.5 + 0.5;
				float4 texCUBENode10 = texCUBE( _PortalCubemap, ase_worldViewDir );
				float smoothstepResult130 = smoothstep( 0.09 , 0.03 , texCUBENode10.r);
				float4 lerpResult42 = lerp( _ColorA , _ColorB , ( simplePerlin3D141 * smoothstepResult130 ));
        ...
        Pass
        {
			...
			)
			{
				...
				float mulTime142 = _TimeParameters.x * 0.1;
				float simplePerlin3D141 = snoise( ( ( mulTime142 * float3(0,1,0) ) + packedInput.viewDirectionWS )*10.0 );
				simplePerlin3D141 = simplePerlin3D141*0.5 + 0.5;
